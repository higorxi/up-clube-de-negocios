...
const Header: React.FC<HeaderProps> = ({ user, onMenuClick }) => {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, message: 'Nova mensagem de cliente', read: false, timestamp: new Date() },
    { id: 2, message: 'Seu pedido foi processado', read: false, timestamp: new Date() },
  ]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (!event.target.closest('.dropdown-container') && !event.target.closest('.notifications-container')) {
    ...
    <header className="bg-white shadow px-4 py-3 flex items-center justify-between lg:justify-end">
      ...
      <div className="flex-1">
        ...
        <p className="text-gray-500 text-sm">
          Bem-vindo(a) ao seu painel de {user.type === 'professional' ? 'profissional' : 'lojista'}
        </p>
      </div>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-60 bg-white rounded-md shadow-lg py-1 z-10 border border-gray-200">
              {notifications.length === 0 ? (
                <p className="px-4 py-2 text-sm text-gray-500">Sem notificações</p>
              ) : (
                notifications.map((notif) => (
                  <div
                    key={notif.id}
                    className={`px-4 py-2 text-sm ${notif.read ? 'text-gray-500' : 'font-medium'} hover:bg-gray-100`}
                    onClick={() => handleNotificationClick(notif.id)}
                  >
                    <div>{notif.message}</div>
                    <div className="text-xs text-gray-400 mt-1">{formatDate(notif.timestamp)}</div>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="relative dropdown-container">
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10 border border-gray-200">
              ...
              <a href="/configuracoes" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                Configurações
              </a>
              <a href="/logout" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 border-t border-gray-100">
                Sair
              </a>
            </div>
          )}
